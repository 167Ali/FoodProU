<!-- src/components/Customer/CheckOut.vue-->
        ...
        <div class="col-lg-5">
          <div class="card p-4">
            ...
            <div class="order-placed-content">
              ...
              <div v-for="item in orderDetails?.order_details?.items" :key="item.menu_item_id" class="order-item d-flex justify-content-between my-1">
                <p>{{ item.quantity }} × {{ item.menu_item_name }}</p>
                <span>Rs. {{ item.total_price }}</span>
              </div>
              <hr />
              <div class="price-breakdown">
                <p class="d-flex justify-content-between">Subtotal <span>Rs. {{ orderDetails?.order_details?.total }}</span></p>
                <!-- <p class="d-flex justify-content-between">Standard delivery <span>Free</span></p> -->
                <p class="d-flex justify-content-between">Standard delivery <span>Rs. {{ orderDetails?.delivery_details?.delivery_fee || 'Free' }}</span></p>
              </div>
              <!--  -->
              <div>
